...
export function AiGenerateDialog({
  ...
  const [selectedTone, setSelectedTone] = useState<string>("Festive");
  ...
  const getDefaultTemplate = (fName?: string, cCode?: string, dPct?: string) => {
    const f = fName || festivalName || "Festival";
    const c = cCode || couponVal || couponCode || "FESTIVE20";
    const d = dPct || discountVal || discountPercent || "20%";
    return `Happy ${f} {customer_name}! 🎉\n\nCelebrate ${f} with us at {restaurant_name}.\nUse coupon ${c} and enjoy ${d} OFF on your festive meal.\n\nWe look forward to serving you! ❤️\nTeam {restaurant_name}`;
  };
  ...
  useEffect(() => {
    ...
    const initialText = initialMessage || getDefaultTemplate(festivalName, couponCode, String(discountPercent));
    setText(initialText);
    if (couponCode) setCouponVal(couponCode);
    if (discountPercent) setDiscountVal(String(discountPercent));
  }, [open, initialMessage, festivalName, couponCode, discountPercent]);

  const fetchAiMessage = async () => {
    setLoading(true);
    setError(null);
    const targetTone = selectedTone;
    const targetLang = selectedLang;
    const targetLength = selectedLength;
    const targetTiming = selectedTiming;
    const targetCoupon = couponVal;
    const targetDisc = discountVal;
    ...
    if (campaignType === "festival" || festivalName) {
      try {
        ...
        if (!res.ok) {
          ...
          const msg = typeof errDetail === "string" ? errDetail : (errDetail?.detail || "AI message generation failed.");
          const err = new Error(msg);
          (err as any).status = res.status;
          (err as any).detail = errDetail;
          throw err;
        }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl sm:max-w-xl border shadow-xl bg-card max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter className="gap-2 sm:justify-between pt-2">
          <Button
            variant="outline"
            className="rounded-full text-xs font-semibold hover:bg-primary/5 hover:text-primary transition-all border-primary/30"
            onClick={fetchAiMessage}
            disabled={loading}
          >
            {loading ? (
              <>
                <Sparkles className="mr-1.5 h-3.5 w-3.5 text-primary animate-spin" />
                Generating...
              </>
            ) : (
              <>
          ...
          <div className="flex gap-2">
            <Button variant="ghost" className="rounded-full text-xs" onClick={() => onOpenChange(false)} disabled={loading}>
              Cancel
            </Button>
            <Button
              className="rounded-full bg-primary text-primary-foreground text-xs font-semibold shadow-sm hover:shadow transition-all"
