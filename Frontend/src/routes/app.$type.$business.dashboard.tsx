...
import {
  ...
  Sparkles, ChevronRight, ListChecks, ShoppingBag, Scissors, Utensils,
  AlertTriangle, PieChart as PieIcon, Activity, TrendingUp, Clock
} from "lucide-react";
...
function BusinessDashboard() {
  ...
  // ---------------------------------------------------------------------------
  // Fetch 100% Database-Driven Dashboard Analytics
  // ---------------------------------------------------------------------------
  const {
    data: dashData,
  ...
  const topCategories = dashData?.top_categories || [];
  ...
  const returningCustomers = dashData?.returning_customers || { today: 0, this_week: 0, this_month: 0 };
  const recentActivity = dashData?.recent_activity || [];
  const calculatedInsights = dashData?.calculated_insights || [];
  const campaignSuggestions = dashData?.campaign_suggestions || [];
      ...
      {isError ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
            <AppLink path="orders">
              <StatCard
                label={type === "salon" ? "Today's Appointments" : "Today's Orders"}
                value={todaysOrders}
                icon={type === "salon" ? Calendar : ShoppingBag}
                accent="primary"
                index={0}
              />
            </AppLink>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Card className="rounded-2xl border shadow-xs">
              <CardHeader className="pb-2">
                <CardTitle className="font-display flex items-center gap-2 text-base">
                  {type === "salon" ? <Scissors className="h-4 w-4 text-primary" /> : <ShoppingBag className="h-4 w-4 text-primary" />}
                  Top selling items
                </CardTitle>
              ...
              <CardContent className="space-y-2">
                {topSellingItems.length === 0 ? (
                  ...
                  topSellingItems.map((it: any, i: number) => (
