...
import { SkeletonCustomerCards, SkeletonRows, useShortMountFlag } from "@/components/skeletons";
import { archiveCustomer, useActiveCustomers } from "@/lib/archive-store";
import { fmt } from "@/lib/currency";
import { custId } from "@/lib/orders-store";
...
function CustomersPage() {
  ...
  const filtered = active.filter((c) => {
    return (status === "all" || c.status === status) && c.name.toLowerCase().includes(q.toLowerCase());
  });

  const target = toArchive ? active.find((c) => c.id === toArchive) : null;

  function handleWhatsApp(c: any) {
  ...
  return (
    <PageTransition>
      <PageHeader
        title="Customers"
        description={`${active.length} active customer${active.length === 1 ? "" : "s"} · CSV import & export supported`}
        actions={
          <>
            ...
            <Button size="sm" className="rounded-full gradient-brand text-primary-foreground transition-transform hover:scale-105 active:scale-95"><Plus className="mr-1.5 h-4 w-4" /> Add customer</Button>
          </>
        }
      />
              ...
              <TableBody>
                {filtered.map((c) => (
                  <TableRow key={c.id} className="group">
                    ...
                    <TableCell className="font-mono text-xs text-muted-foreground">{custId(c.id)}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex justify-end gap-1">
                        ...
                        <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full" title="Edit" onClick={() => toast("Edit dialog")}><Edit className="h-3.5 w-3.5" /></Button>
      ...
      <ConfirmDialog
        ...
        description="They will be hidden from active lists. Archived customers can be restored later — nothing is permanently deleted."
        confirmLabel="Archive customer"
        destructive
        onConfirm={() => {
          if (toArchive) {
            archiveCustomer(toArchive);
